...
// tag::init[]
fun sumL(xs: List<Int>): Int = foldLeft(xs, 0) { x, y -> y + x  }

fun productL(xs: List<Double>): Double = foldLeft(xs, 1.0) { x, y -> y * x  }

fun <A> lengthL(xs: List<A>): Int = foldLeft(xs, 0) { x, y -> x + 1  }
// end::init[]
